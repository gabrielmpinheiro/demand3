...
export default function DefaultLayout() {
    const { user, cliente, token, setUser, setCliente, setToken, notificationCount, setNotificationCount, theme, setTheme } = useStateContext();
    ...
    const navItems = [
        ...
        // Parceria: apenas para clientes parceiros
        ...(user?.cliente?.is_parceiro ? [{ to: '/partnership', label: 'Parceria', icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z', badge: 'Parceiro' }] : []),
    ];
                ...
                <nav className="mt-4 flex-1 overflow-y-auto pb-20">
                    {navItems.map((item) => (
                        ...
                        >
                            ...
                            {item.badge && (
                                <span className="ml-auto bg-purple-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">{item.badge}</span>
                            )}
                        </Link>
                    ))}
                </nav>
