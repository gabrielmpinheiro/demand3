...
export default function Payments() {
    ...
    const handleGerarMensalidades = async () => {
        if (!window.confirm('Gerar mensalidades pendentes para o mês atual?')) return;
        setGeneratingInvoices(true);
        try {
            const { data } = await axiosClient.post('/pagamentos/gerar-mensalidades-pendentes');
            showNotification(data.message || 'Mensalidades geradas com sucesso');
            await fetchPayments();
        } catch (error) {
            const errorMsg = error.response?.data?.message || 'Erro ao gerar mensalidades';
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                ...
                <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                    ...
                    <button
                        ...
                        title="Verifica e gera mensalidades faltantes para o mês atual"
                    >
                        {generatingInvoices ? (
